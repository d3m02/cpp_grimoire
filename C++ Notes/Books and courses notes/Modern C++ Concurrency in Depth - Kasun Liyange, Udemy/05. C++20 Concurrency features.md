...
_Coroutines_ - similar to subroutines, but with addition of two stage - _suspend_ and _resume_. When coroutine object is initialized - it's in suspended state and should be explicitly resumed for execution. Coroutines constructed in heap (instead a stack for subroutines). When from caller function coroutine execution resumed - it creates frame copy on caller stack. When execution suspended, heap frame will be updated and the temporary stack frame popped from the caller stack. 
...
To use coroutines we need to define handle object, promise. In most cases it will use similar boilerplate. 
