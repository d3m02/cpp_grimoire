...
public: 
    ...
    void push(T new_value) {
	    // avoid expensive allocation under mutex
...
T parallel_accumulate(Iterator begin, Iterator end, T init) {
    ...
    std::future<T> future = std::async(parallel_accumulate<Iterator, T>, mid, end, T{});
    auto sum = parallel_accumulate(begin, mid, init);
    return sum + future.get();
}
```
> [!note] 
> This example actually show bad performance even compare to sequential `std::accumulate` - it doesn't take in account possible exponential growth of threads during performing recursion. Results can be improved if limit depth of recursion
> ```cpp fold:"Improved example"
>template<typename Iterator, typename T>
>T parallel_accumulate_async(Iterator begin, Iterator end, T init, unsigned depth = 0) {
>    const auto inputSize = std::distance(begin, end);
>
>    if (inputSize == 0)
>        return init;
...
>
>    if (depth >= maxDepth || inputSize < min_block_size)
>        return std::accumulate(begin, end, init);
>
>    Iterator mid = begin;
>    std::advance(mid, (inputSize + 1) / 2);
>
>    std::future<T> future = std::async(std::launch::async, parallel_accumulate_async<Iterator, T>, mid, end, T{}, depth + 1);
>    auto sum = parallel_accumulate_async(begin, mid, init, depth + 1);
>    return sum + future.get();
>}
> ```
> Yet, `std::reduce` from C++17 with `std::execution::par` can outperform both examples, with `std::thread` and with `std::async`
> ```
> // constexpr auto DATA_SIZE = 500'000'000;
>seq std::reduce 208ms
>par std::reduce 179ms
>Parallel accumulate with blocks and thread 181ms
>Parallel accumulate with recursion and async 180ms
> ```
