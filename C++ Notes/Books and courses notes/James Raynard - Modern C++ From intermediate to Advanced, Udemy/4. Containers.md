...
The C++ `std::stack` presented in `<stack>` header, it's using by default as underlaying container `std::vector`, also possible to use `std::deque`, `std::list` and. `std::vector<bool>`. 
